...
vec3 calculateLighting(vec3 fragPos, vec3 normal, vec3 albedo, float shininess, vec3 materialAmbient, float viewDistance, float precise_view_z, float depth, out vec3 totalAmbient) {
    ...
    for(int i=0; i < maximumPointLights; ++i){
        LightSource currentLight = LightSources.lights[i];
        if(currentLight.type != 0) {
            vec3 lightDirectory;
            if(currentLight.type == 1) { // Directional Light
                lightDirectory = normalize(-currentLight.position);
            } else if(currentLight.type == 2) { // Point Light
                lightDirectory = normalize(currentLight.position - fragPos);
            }
            ...
            directLighting += ((1.0 - shadow) * (diffuseRate + specularRate) * currentLight.color);
            lightAmbient += currentLight.ambient;
        }
    }
