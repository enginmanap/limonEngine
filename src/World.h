...
private:
    ...
    std::unordered_map<Camera*, std::unordered_map<std::vector<uint64_t>, std::unordered_map<uint32_t , std::pair<std::multimap<float, glm::uvec4>, uint32_t>>, VisibilityRequest::uint64_vector_hasher>*> cullingResults2;
    //what do we actually want in the end?
    ...
    struct PerMeshRenderInformation {
        ...
        uint32_t lod;  //Max level of detail. Since we use instanced rendering, using single LOD for all meshes is faster than multiple draw calls (at least in my testing)
    ...
    struct compareMeshRenderInformationByDepth {
        bool operator()( const std::pair<std::shared_ptr<MeshAsset>,const PerMeshRenderInformation>& a, const std::pair<std::shared_ptr<MeshAsset>,const PerMeshRenderInformation>& b) const {
