...
public:
    Glyph(GLHelper* glHelper, FT_Face face, const int size, const char character):
        ...
        if (FT_Load_Char(face, character, FT_LOAD_RENDER)) {
            ...
            textureID = glHelper->loadTexture(face->glyph->bitmap.rows, face->glyph->bitmap.width, GL_RED, face->glyph->bitmap.buffer);
...
public:
    Face(GLHelper* glHelper, std::string path , int size, FT_Face face): glHelper(glHelper), path(path), size(size), face(face) {}
